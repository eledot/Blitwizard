...
int so_SelectWait(int maximumsleeptime);
   ...
   //Please check ALL sockets you marked subsequently with so_SelectSaysWrite() and
   //so_SelectSaysRead(), and if any of them is told to have something to be read or
   //written, read/write your things!
...
/* === SSL socket handling === */
//NOTE: all those functions perfectly work on normal sockets if you simply don't provide an sslptr
// (just set NULL for sslptr (accept) or NULL for either sslptr/*sslptr for the other functions)
